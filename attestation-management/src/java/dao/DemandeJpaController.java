...
public class DemandeJpaController implements Serializable {
    ...
    public void create(Demande demande) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            em.persist(demande);
            if (userId != null) {
                userId.getDemandeCollection().add(demande);
                userId = em.merge(userId);
            }
            if (etat != null) {
                etat.getDemandeCollection().add(demande);
                etat = em.merge(etat);
            }
            utx.commit();
        } catch (Exception ex) {
    ...
    public void edit(Demande demande) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            if (userIdOld != null && !userIdOld.equals(userIdNew)) {
                userIdOld.getDemandeCollection().remove(demande);
                userIdOld = em.merge(userIdOld);
            }
            if (userIdNew != null && !userIdNew.equals(userIdOld)) {
                userIdNew.getDemandeCollection().add(demande);
                userIdNew = em.merge(userIdNew);
            }
            if (etatOld != null && !etatOld.equals(etatNew)) {
                etatOld.getDemandeCollection().remove(demande);
                etatOld = em.merge(etatOld);
            }
            if (etatNew != null && !etatNew.equals(etatOld)) {
                etatNew.getDemandeCollection().add(demande);
    ...
    public void destroy(Integer id) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            User userId = demande.getUserId();
            if (userId != null) {
                userId.getDemandeCollection().remove(demande);
                userId = em.merge(userId);
            }
            Statusdemande etat = demande.getEtat();
            if (etat != null) {
                etat.getDemandeCollection().remove(demande);
                etat = em.merge(etat);
            }
            em.remove(demande);
            utx.commit();
